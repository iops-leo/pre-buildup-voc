...
export const MultiplicationAdventureView = ({ onBack }: MultiplicationAdventureViewProps) => {
    ...
    const progress = Math.min(((questionIndex + (answered ? 1 : 0)) / activePreset.questionCount) * 100, 100);
    ...
    return (
        <div className="w-full max-w-4xl mx-auto p-4 md:p-6 space-y-5 animate-in fade-in">
            <div className="flex flex-col gap-4 rounded-[28px] border border-slate-800 bg-slate-900/90 p-4 md:p-5">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="text-xs font-bold uppercase tracking-[0.28em] text-slate-500">Gugudan Quest</div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 md:justify-end">
                        ...
                        <div className="rounded-full border border-slate-700 bg-slate-950 px-3 py-1.5 text-sm font-bold text-orange-200">
                            🔥 {combo}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div className="h-3 flex-1 overflow-hidden rounded-full bg-slate-950">
                        <motion.div
                            className={clsx('h-full bg-gradient-to-r', presetStyle.progress)}
                    ...
                    <div className="flex items-center gap-2">
                        {Array.from({ length: activePreset.lives }).map((_, index) => (
                            <Heart
                                ...
                                size={20}
                                className={clsx(
                                    index < lives ? 'text-rose-400 fill-rose-400' : 'text-slate-700'
                                )}
                            />
                        ))}
                    </div>
            ...
            <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
                <div className="space-y-5">
                    <div className={clsx('rounded-[28px] border p-5 md:p-6', presetStyle.card)}>
                        <div className="flex items-start justify-between gap-4">
                            <div>
                                <div className={clsx('inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-bold', presetStyle.chip)}>
                                    <Sparkles size={14} />
                                    {currentQuestion?.dan}단 집중 미션
                                </div>
                                <p className="mt-3 text-sm text-slate-200/90">{buddyMessage}</p>
                            </div>
                            <div className="rounded-3xl border border-white/15 bg-slate-950/40 px-4 py-3 text-center">
                                <div className="text-3xl">{activePreset.mascot}</div>
                                <div className="mt-1 text-xs font-bold text-slate-300">점수 {score}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <AnimatePresence mode="wait">
                        {currentQuestion && (
                            <motion.div
                                ...
                                className="rounded-[32px] border border-slate-800 bg-slate-900/90 p-6 md:p-8 shadow-2xl shadow-black/20"
                            >
                                <div className="text-center">
                                    <div className="text-xs font-bold uppercase tracking-[0.36em] text-slate-500">Question</div>
                                    <div className="mt-4 text-5xl md:text-7xl font-black text-white">
                                        {currentQuestion.expression}
                                        <span className="text-amber-300"> = ?</span>
                                    </div>
                                    <p className="mt-4 text-lg text-slate-300">{currentQuestion.hint}</p>
                                </div>

                                <div className="mt-6">
                                    <div className="mb-3 text-center text-xs font-bold uppercase tracking-[0.28em] text-slate-500">
                                        건너뛰기 셈
                                    </div>
                                    <div className="flex flex-wrap justify-center gap-2">
                                        {currentQuestion.skipSequence.map((value, index) => (
                                            <span
                                                key={`${currentQuestion.key}-${value}`}
                                                className={clsx(
                                                    'rounded-full border px-3 py-1.5 text-sm font-bold',
                                                    index === currentQuestion.skipSequence.length - 1
                                                        ? `shadow-lg ${presetStyle.chip}`
                ...
                <div className="space-y-4">
                    <div className="rounded-[28px] border border-slate-800 bg-slate-900/90 p-5">
                        <div className="flex items-center justify-between">
                            <div>
                                <h2 className="text-lg font-black text-white">정답 고르기</h2>
                                <p className="text-sm text-slate-400">숫자 키 `1`~`4`로도 풀 수 있어요.</p>
                            </div>
                            {answered && (
                                <div className={clsx(
                                ...
                                return (
                                    <button
                                        ...
                                        className={clsx(
                                            ...
                                            !answered && 'border-slate-700 bg-slate-950 text-white hover:-translate-y-0.5 hover:border-slate-500 hover:bg-slate-800'
                                        )}
                                    >
                                        <div className="text-xs font-bold text-slate-500">{index + 1}번</div>
                    ...
                    <div className="rounded-[28px] border border-slate-800 bg-slate-900/90 p-5">
                        <h3 className="text-lg font-black text-white">학습 포인트</h3>
                        <ul className="mt-4 space-y-3 text-sm text-slate-400">
                            <li className="flex items-start gap-3">
                                <CheckCircle size={18} className="mt-0.5 text-emerald-400 shrink-0" />
                                <span>같은 단은 숫자가 일정하게 커져요. 건너뛰기 셈을 같이 보면서 외워요.</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <Star size={18} className="mt-0.5 text-amber-400 shrink-0" />
                                <span>정답을 맞힌 뒤 문제를 한 번 더 소리 내서 읽으면 기억이 더 오래가요.</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <XCircle size={18} className="mt-0.5 text-rose-400 shrink-0" />
                                <span>틀린 문제는 끝에서 다시 보여줘서, 바로 복습할 수 있게 했어요.</span>
                            </li>
                        </ul>
                    </div>
                </div>
